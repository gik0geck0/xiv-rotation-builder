...
export default class HelloWorldApp extends LightningElement {
    ...
    findTimes(actionList) {
        ...
        for (let i = 1; i < actionList.length; i++) {
            ...
            if (
                currAction.type == 'Spell' ||
                currAction.type == 'Weaponskill'
            ) {
                ...
                //if not, add cast time
                    ...
                    if (currTime <= lastGCD[1] + GCDTime && lastGCD[1] != -1) {
                        currTime =
                            lastGCD[1] + GCDTime + parseFloat(currAction.cast);
                    } else {
            ...
            if (currAction.cast != 'Instant') {
                ...
                if (
                    parseFloat(currAction.recast) >= parseFloat(currAction.cast)
                ) {
                ...
                //otherwise, it will just the the time before the skill plus recast
                    ...
                    usedActions.push([
                        currAction.name,
                        lastTime + parseFloat(currAction.recast)
                    ]);
                }
            }
        ...
        for (let i = 0; i < timedList.length; i++) {
            ...
            if (hasOwnProperty(currAction, 'damageBuff')) {
                currBuffs.push([
                    currAction,
                    currTime,
                    currTime + parseFloat(currAction.duration)
                ]);
            }
            ...
            if (hasOwnProperty(currAction, 'grants')) {
                for (
                    let k = 0;
                    k < Object.keys(currAction.grants).length;
                    k++
                ) {
                    if (
                        parseInt(
                            currAction.grants[Object.keys(currAction.grants)[k]]
                        ) != -1
                    ) {
                        currBuffs.push([
                            ...
                            parseFloat(
                                currAction.grants[
                                    Object.keys(currAction.grants)[k]
                                ]
                            ),
                            currTime,
                            currTime + 30
            ...
            if (lastAction != null) {
                if (
                    hasOwnProperty(currAction, 'comboBonus') &&
                    currAction.comboAction == lastAction.name
                ) {
                    for (
                        let k = 0;
                        k < Object.keys(currAction.comboBonus).length;
                        k++
                    ) {
                        if (
                            parseInt(
                                currAction.comboBonus[
                                    Object.keys(currAction.comboBonus)[k]
                                ]
                            ) != -1
                        ) {
                            currBuffs.push([
                                Object.keys(currAction.comboBonus)[
                                    k
                                ].toLowerCase(),
                                parseFloat(
                                    currAction.comboBonus[
    ...
    calculatePotency(timedList) {
        ...
        */
        ...
        for (let i = 0; i < timedList.length; i++) {
            ...
            for (let j = 0; j < currBuffs.length; j++) {
                ...
                //4 length means a stack based buff
                    ...
                    if (currBuffs[j][1] >= 1) {
                        if (hasOwnProperty(currAction, [currBuffs[j][0]])) {
                            extraPotency = currBuffs[j][0];
                        }
                    }
                    stacksUsed = 0;
            ...
            if (
                ...
                if (extraPotency != null) {
                    totalPotency +=
                        parseFloat(currAction[extraPotency]) * buffAmt;
                    stacksUsed = -1;
                }
                //if there is a combo potency
                else if (
                    currAction.comboAction == lastAction.name &&
                    hasOwnProperty(currAction, 'comboAction')
                ) {
                    totalPotency +=
                        parseFloat(currAction.comboPotency) * buffAmt;
                }
                //normal potency
        ...
        this.template.querySelector('lightning-card.potencyLabel').title =
            'Total Potency: ' + totalPotency;
        this.template.querySelector('lightning-card.ppsLabel').title =
    ...
    validation(actionList, job) {
        ...
        if (actionList.length === 0) {
            this.template.querySelector('lightning-card.potencyLabel').title =
                'Total Potency: Add actions to recieve a potency.';
            this.template.querySelector('lightning-card.ppsLabel').title =
                'Potency Per Second: Add actions to recieve a pps.';
        } else {
            ...
            for (let i = 0; i < timedList.length; i++) {
                ...
                if (i == 0) {
                    ...
                    actionList[i - 1].timeTaken =
                        actionList[i].startTime - actionList[i - 1].startTime;
                }
            ...
            for (let i = 0; i < timedList.length; i++) {
                ...
                for (let j = 0; j < gaugeAmounts.length; j++) {
                    ...
                    if (currAction.cast === 'Instant') {
                        gaugeAmounts[Object.keys(gaugeAmounts[j])] =
                            5 * Math.floor((timedList[i][1] - 0.7) / 2.5);
                    } else {
                        gaugeAmounts[Object.keys(gaugeAmounts[j])] =
                            5 *
                            Math.floor(
                                (timedList[i][1] - currAction.cast) / 2.5
                            );
                    }

                    //Checking against new gauge amounts for enough to cast
                    var gaugeName = Object.keys(gaugeAmounts[j])[0];
                    if (hasOwnProperty(currAction, gaugeName)) {
                        if (
                            gaugeAmounts[Object.keys(gaugeAmounts[j])] +
                                currAction[gaugeName] <
                            0
                        ) {
                ...
                if (hasOwnProperty(currAction, 'buffRequirement')) {
                    var buffCheck = 0;
                    ...
                    for (let j = 0; j < buffList.length; j++) {
                        ...
                        if (buffList[j][0] === currAction.buffRequirement) {
                            ...
                            if (buffList[j].length === 4) {
                                //Stack buff
                                if (
                                    timedList[i][1] < buffList[j][2] ||
                                    timedList[i][1] > buffList[j][3]
                                ) {
                                    invalidActionList.push([
                                        currAction,
                                        i,
            ...
            if (invalidActionList.length > 0) {
                ...
                for (let i = 0; i < invalidActionList.length; i++) {
                    ...
                    actionList[invalidActionList[i][1]].errorMessage =
                        invalidActionList[i][2];
                }
            }
    ...
    addTimelineAction(e) {
        ...
        this.mockActionList.push(
            JSON.parse(
                JSON.stringify(getActionInfo(this.job, e.detail.actionName))
            )
        );
    ...
    spliceTimelineAction(e) {
        ...
        const movedItem = this.mockActionList.splice(

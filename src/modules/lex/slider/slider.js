...
import { getDecimalPlaces } from 'lex/numberUtils';
import { numberFormat } from 'lex/internationalizationLibrary';
import {
    InteractingState,
    normalizeVariant,
...
export default class LightningSlider extends LightningElement {
    ...
    get computedClass() {
        ...
        const classes = ['slds-slider'];

        if (normalizedType === 'vertical') {
            classes.push('slds-slider_vertical');
        }
        if (normalizedSize) {
            classes.push(`slds-size_${normalizedSize}`);
        }
        return classes.join(' ');
    }
